...
fun main() {
    ...
    val set = arrayOf(
        ...
        arrayOf(4, 42, 16, 73, 38, 25, 39, 11, 24, 94, 72, 18, 8, 46, 29, 32 ,40, 62, 76, 36),//16
    ...
    fun horizontal(x:Int,y:Int): Long{

        var value = 1L
        if (x >= 17) return value
...
//            print(" "+set[y][it])
    ...
    fun vertical(x:Int, y:Int): Long{
        var value = 1L
        if (y >= 17) return value
    ...
    fun diagonal(x:Int, y:Int): Long{
        var value = 1L
        if (x>=17 || y>=17) return value
    ...
    fun reversedDiagonal(x:Int, y:Int): Long{
        var value = 1L
        if (x<3 || y>=17) return value
...
//            ciba += vertical(x,y)
//            ciba += diagonal(x,y)
//            ciba += reversedDiagonal(x,y)
            if (horizontal(x, y) > max)
                max = horizontal(x, y)
            if (vertical(x, y) > max) max = vertical(x, y)
            if (diagonal(x, y) > max) max = diagonal(x, y)
            if (reversedDiagonal(x, y) > max) max = reversedDiagonal(x,y)
        }
    }

//    print(ciba.sortedDescending())
    println(max)
}
